...
import { Goal, RoadmapStep, RoadmapSubstep, RoomItem } from "@shared/schema";
import { Sheet, SheetContent, SheetHeader, SheetTitle, SheetDescription } from "@/components/ui/sheet";
import { useLogs, useCreateLog, useUpdateGoal } from "@/hooks/use-goals";
import { format } from "date-fns";
import { nl } from "date-fns/locale";
import { Loader2, Plus, Minus, Check, Calendar, MessageSquare } from "lucide-react";
...
export function GoalDetailSheet({ goal, open, onOpenChange }: GoalDetailSheetProps) {
  ...
  const renderLogItem = (log: any) => {
        ...
        <div className="flex items-center justify-between p-3 bg-secondary/50 rounded-xl">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">
              {log.itemTitle || `Item ${log.value > 0 ? "afgerond" : "ongedaan gemaakt"}`}
            </span>
          </div>
    ...
    if (goal.type === "roadmap") {
      return (
        <div className="flex items-center justify-between p-3 bg-secondary/50 rounded-xl">
          <div className="flex items-center gap-2">
            <Check className="w-4 h-4 text-blue-500" />
            <span className="text-sm">
              {log.stepTitle || `Stap ${log.value > 0 ? "voltooid" : "ongedaan gemaakt"}`}
            </span>
          </div>
          <span className="text-xs text-muted-foreground">
            {format(new Date(log.createdAt || ""), "d MMM HH:mm", { locale: nl })}
      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto bg-background">
        ...
        {isLoading ? (
          ...
          <div className="space-y-5">
            {(goal.type === "counter" || goal.type === "progress") && (
            ...
            {goal.type === "roadmap" && goal.metadata && (
              <div className="bg-card rounded-xl border border-border p-4">
                <h3 className="font-semibold text-sm mb-3">Stappen</h3>
                <div className="space-y-2">
